...
//
// Phase 33 note: All includes in this file are from core Build.cs modules (Engine, AssetTools,
// AssetRegistry). No optional plugin headers are needed for material operations -- UMaterialInterface,
// UMaterialInstanceConstant, UPrimitiveComponent, and UStaticMesh are all core Engine types.
// ReflectionHelpers.h is included for MCPReflect::CheckModuleLoaded and the shared response
// builders (used indirectly via module availability pattern).
//
// Threat mitigations applied:
//   T-15-01 / T-33-01: param_type uses explicit allowlist; only "scalar"/"vector"/"texture" accepted.
//   T-15-02 / T-33-01: instance_path validated to start with "/Game/"; engine asset overwrite prevented.
//   T-15-04 / T-33-02: texture StaticLoadObject null-checked before SetTextureParameterValue.
...
void RegisterMaterialCommands(FMCPCommandRouter& Router)
{
	...
	//
	// Threat T-15-02 / T-33-01: instance_path validated to start with "/Game/".
	// -----------------------------------------------------------------------
	Router.RegisterHandler(TEXT("material.createInstance"), [](TSharedPtr<FJsonObject> Cmd, FMCPResponseSender SendResponse)
	{
		...
		// Threat T-15-02 / T-33-01: Validate instance_path starts with "/Game/" to prevent overwriting engine assets.
	...
	//
	// Threat T-15-01 / T-33-01: param_type uses explicit allowlist.
	// Threat T-15-04 / T-33-02: texture null-checked before SetTextureParameterValue.
	// -----------------------------------------------------------------------
	Router.RegisterHandler(TEXT("material.setParam"), [](TSharedPtr<FJsonObject> Cmd, FMCPResponseSender SendResponse)
	{
		...
		// Threat T-15-01 / T-33-01: Explicit allowlist for param_type.
		...
		else // bIsTexture
		{
			...
			{
				// Threat T-15-04 / T-33-02: null-check before calling SetTextureParameterValueEditorOnly.
