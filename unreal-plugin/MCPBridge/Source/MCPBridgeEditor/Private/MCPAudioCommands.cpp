// MCPAudioCommands.cpp (Plan 33-03)
...
// (Plan 33-03). MetaSound-specific plugin headers eliminated -- these types are
// accessed at runtime via FindObject<UClass> and FProperty reflection.

#include "MCPAudioCommands.h"
#include "ReflectionHelpers.h"

// Core Sound headers (Engine module -- always available)
...
void RegisterAudioCommands(FMCPCommandRouter& Router)
{
	...
	// MetaSound assets listed via FTopLevelAssetPath with reflected class path (T-33-07).
	// -----------------------------------------------------------------------
	Router.RegisterHandler(TEXT("audio.list"), [](TSharedPtr<FJsonObject> Cmd, FMCPResponseSender SendResponse)
	{
		...
		// Query MetaSound assets via reflection (no MetasoundEngine header required).
		// Use FTopLevelAssetPath with the scripted class path -- the asset registry uses
		// class path strings, not UClass pointers, so this works without a loaded module.
		if (bFilterMetaSound)
		{
			TArray<FAssetData> AssetList;
	...
	//
	// MetaSound types accessed via UE reflection (no MetasoundEngine headers):
	//   - UMetaSoundSource found via FindObject<UClass>(nullptr, ".../MetaSoundSource")
	//   - FMetasoundFrontendDocument/Graph/Node/Edge read via FProperty iteration
	//   - Array properties (Nodes, Edges, Inputs, Outputs) read via FArrayProperty + FScriptArrayHelper
	// -----------------------------------------------------------------------
	Router.RegisterHandler(TEXT("audio.metasound"), [](TSharedPtr<FJsonObject> Cmd, FMCPResponseSender SendResponse)
	{
		...
		// Validate path prefix (T-23-01, T-33-07).
		...
		// Check MetasoundEngine module is available at runtime.
		if (!MCPReflect::CheckModuleLoaded(TEXT("MetasoundEngine")))
		{
			MCPReflect::SendModuleNotAvailable(SendResponse, CorrId, TEXT("MetasoundEngine"));
			return;
		}

		// Find the MetaSoundSource UClass via reflection (no #include required).
		UClass* MetaSoundClass = MCPReflect::FindClassByPath(TEXT("/Script/MetasoundEngine.MetaSoundSource"));
		if (!MetaSoundClass)
		{
			SendResponse(BuildAudioErrorResponse(CorrId, TEXT("metasound_class_not_found")) + TEXT("\n"));
			return;
		}

		// Load the MetaSoundSource asset as UObject* (no typed pointer needed).
		UObject* MetaSoundAsset = StaticLoadObject(MetaSoundClass, nullptr, *AssetPath);
		if (!MetaSoundAsset)
		{
		...
		// Access the MetaSound document via FProperty reflection.
		// UMetaSoundSource stores its document in a property named "RootDocumentProperty" or
		// via a nested struct. We iterate the class properties to find the FMetasoundFrontendDocument
		// struct property, then extract nodes, edges, inputs, and outputs from sub-structs.
		//
		// MetaSound document structure (via UPROPERTY reflection):
		//   UMetaSoundSource::RootDocumentProperty -> FMetasoundFrontendDocument
		//     FMetasoundFrontendDocument::RootGraph -> FMetasoundFrontendGraphClass
		//       FMetasoundFrontendGraphClass::Graph (or GraphPages) -> FMetasoundFrontendGraph[]
		//         FMetasoundFrontendGraph::Nodes -> FMetasoundFrontendNode[]
		//         FMetasoundFrontendGraph::Edges -> FMetasoundFrontendEdge[]
		//       FMetasoundFrontendGraphClass::Interface -> FMetasoundFrontendClassInterface
		//         FMetasoundFrontendClassInterface::Inputs -> FMetasoundFrontendClassInput[]
		//         FMetasoundFrontendClassInterface::Outputs -> FMetasoundFrontendClassOutput[]
		//
		// We use FStructProperty + FArrayProperty + FScriptArrayHelper to traverse this.
		...
		// Helper lambda to read a FGuid-like struct as string (for node IDs).
		auto ReadStructAsString = [](void* StructPtr, UScriptStruct* Struct) -> FString
		{
			if (!StructPtr || !Struct)
			{
				return FString();
			...
			FString Result;
			Struct->ExportText(Result, StructPtr, nullptr, nullptr, PPF_None, nullptr);
			return Result;
		};

		// Find the document struct property on the MetaSoundSource class.
		// In UE 5.7 MetaSound, the document is stored as a private UPROPERTY.
		// Property name varies by UE version; we iterate to find a struct property
		// that contains a "RootGraph" sub-property.
		FStructProperty* DocumentProp = nullptr;
		for (TFieldIterator<FStructProperty> It(MetaSoundClass); It; ++It)
		{
			// Look for FMetasoundFrontendDocument struct by checking for RootGraph sub-prop.
			UScriptStruct* Struct = It->Struct;
			if (Struct && Struct->FindPropertyByName(FName(TEXT("RootGraph"))))
			{
				DocumentProp = *It;
				break;
			}
		...
		if (DocumentProp)
		{
			void* DocumentPtr = DocumentProp->ContainerPtrToValuePtr<void>(MetaSoundAsset);
			UScriptStruct* DocumentStruct = DocumentProp->Struct;

			// Find RootGraph property inside the document struct.
			FStructProperty* RootGraphProp = CastField<FStructProperty>(
				DocumentStruct->FindPropertyByName(FName(TEXT("RootGraph"))));

			if (RootGraphProp && DocumentPtr)
			{
				void* RootGraphPtr = RootGraphProp->ContainerPtrToValuePtr<void>(DocumentPtr);
				UScriptStruct* RootGraphStruct = RootGraphProp->Struct;

				// --- Nodes and Edges from Graph/GraphPages ---
				// Try "Graph" property (older UE versions) first, then "GraphPages" (UE 5.5+).
				auto ExtractGraphNodesEdges = [&](void* GraphPtr, UScriptStruct* GraphStruct)
				{
					if (!GraphPtr || !GraphStruct) return;

					// Read Nodes array.
					FArrayProperty* NodesProp = CastField<FArrayProperty>(
						GraphStruct->FindPropertyByName(FName(TEXT("Nodes"))));
					if (NodesProp)
					{
						FScriptArrayHelper NodesHelper(NodesProp, NodesProp->ContainerPtrToValuePtr<void>(GraphPtr));
						FStructProperty* NodeElemProp = CastField<FStructProperty>(NodesProp->Inner);
						if (NodeElemProp)
						{
							UScriptStruct* NodeStruct = NodeElemProp->Struct;
							for (int32 i = 0; i < NodesHelper.Num(); ++i)
							{
								void* NodePtr = NodesHelper.GetRawPtr(i);
								TSharedPtr<FJsonObject> NodeObj = MakeShared<FJsonObject>();

								// Read node_name (FName property)
								if (FNameProperty* NameProp = CastField<FNameProperty>(NodeStruct->FindPropertyByName(FName(TEXT("Name")))))
								{
									NodeObj->SetStringField(TEXT("node_name"), NameProp->GetPropertyValue_InContainer(NodePtr).ToString());
								}

								// Read node_id (struct like FGuid -- export as string)
								if (FStructProperty* IDProp = CastField<FStructProperty>(NodeStruct->FindPropertyByName(FName(TEXT("ID")))))
								{
									void* IDPtr = IDProp->ContainerPtrToValuePtr<void>(NodePtr);
									FString IDStr;
									IDProp->Struct->ExportText(IDStr, IDPtr, nullptr, nullptr, PPF_None, nullptr);
									NodeObj->SetStringField(TEXT("node_id"), IDStr);
								}

								// Read node_class_name (ClassID -- struct or FGuid)
								if (FStructProperty* ClassIDProp = CastField<FStructProperty>(NodeStruct->FindPropertyByName(FName(TEXT("ClassID")))))
								{
									void* ClassIDPtr = ClassIDProp->ContainerPtrToValuePtr<void>(NodePtr);
									FString ClassIDStr;
									ClassIDProp->Struct->ExportText(ClassIDStr, ClassIDPtr, nullptr, nullptr, PPF_None, nullptr);
									NodeObj->SetStringField(TEXT("node_class_name"), ClassIDStr);
								}
					...
					// Read Edges array.
					FArrayProperty* EdgesProp = CastField<FArrayProperty>(
						GraphStruct->FindPropertyByName(FName(TEXT("Edges"))));
					if (EdgesProp)
					{
						FScriptArrayHelper EdgesHelper(EdgesProp, EdgesProp->ContainerPtrToValuePtr<void>(GraphPtr));
						FStructProperty* EdgeElemProp = CastField<FStructProperty>(EdgesProp->Inner);
						if (EdgeElemProp)
						{
							UScriptStruct* EdgeStruct = EdgeElemProp->Struct;
							for (int32 i = 0; i < EdgesHelper.Num(); ++i)
							{
								void* EdgePtr = EdgesHelper.GetRawPtr(i);
								TSharedPtr<FJsonObject> EdgeObj = MakeShared<FJsonObject>();

								auto ExportStructField = [&](const TCHAR* FieldName) -> FString
								{
									FStructProperty* FieldProp = CastField<FStructProperty>(EdgeStruct->FindPropertyByName(FName(FieldName)));
									if (!FieldProp) return FString();
									void* FieldPtr = FieldProp->ContainerPtrToValuePtr<void>(EdgePtr);
									FString Out;
									FieldProp->Struct->ExportText(Out, FieldPtr, nullptr, nullptr, PPF_None, nullptr);
									return Out;
								};

								EdgeObj->SetStringField(TEXT("from_node"),   ExportStructField(TEXT("FromNodeID")));
								EdgeObj->SetStringField(TEXT("from_output"), ExportStructField(TEXT("FromVertexID")));
								EdgeObj->SetStringField(TEXT("to_node"),     ExportStructField(TEXT("ToNodeID")));
								EdgeObj->SetStringField(TEXT("to_input"),    ExportStructField(TEXT("ToVertexID")));
				...
				// Try "GraphPages" array first (UE 5.5+ API).
				FArrayProperty* GraphPagesProp = CastField<FArrayProperty>(
					RootGraphStruct->FindPropertyByName(FName(TEXT("GraphPages"))));
				if (GraphPagesProp)
				{
					FScriptArrayHelper PagesHelper(GraphPagesProp, GraphPagesProp->ContainerPtrToValuePtr<void>(RootGraphPtr));
					FStructProperty* PageElemProp = CastField<FStructProperty>(GraphPagesProp->Inner);
					if (PageElemProp)
					{
						for (int32 i = 0; i < PagesHelper.Num(); ++i)
						{
							ExtractGraphNodesEdges(PagesHelper.GetRawPtr(i), PageElemProp->Struct);
						}
					}
				}
				else
				{
					// Fallback: try single "Graph" struct property.
					FStructProperty* GraphProp = CastField<FStructProperty>(
						RootGraphStruct->FindPropertyByName(FName(TEXT("Graph"))));
					if (GraphProp)
					{
						void* GraphPtr = GraphProp->ContainerPtrToValuePtr<void>(RootGraphPtr);
						ExtractGraphNodesEdges(GraphPtr, GraphProp->Struct);
					}
				}

				// --- Inputs and Outputs from Interface property ---
				// Try "DefaultInterface" (UE 5.6+ API) first, then "Interface" (older).
				FStructProperty* InterfaceProp = CastField<FStructProperty>(
					RootGraphStruct->FindPropertyByName(FName(TEXT("DefaultInterface"))));
				if (!InterfaceProp)
				{
					InterfaceProp = CastField<FStructProperty>(
						RootGraphStruct->FindPropertyByName(FName(TEXT("Interface"))));
				}

				if (InterfaceProp)
				{
					void* InterfacePtr = InterfaceProp->ContainerPtrToValuePtr<void>(RootGraphPtr);
					UScriptStruct* InterfaceStruct = InterfaceProp->Struct;

					auto ExtractClassPins = [&](const TCHAR* ArrayPropName, TArray<TSharedPtr<FJsonValue>>& OutArray)
					{
						FArrayProperty* PinsProp = CastField<FArrayProperty>(
							InterfaceStruct->FindPropertyByName(FName(ArrayPropName)));
						if (!PinsProp) return;

						FScriptArrayHelper PinsHelper(PinsProp, PinsProp->ContainerPtrToValuePtr<void>(InterfacePtr));
						FStructProperty* PinElemProp = CastField<FStructProperty>(PinsProp->Inner);
						if (!PinElemProp) return;

						UScriptStruct* PinStruct = PinElemProp->Struct;
						for (int32 i = 0; i < PinsHelper.Num(); ++i)
						{
							void* PinPtr = PinsHelper.GetRawPtr(i);
							TSharedPtr<FJsonObject> PinObj = MakeShared<FJsonObject>();

							// Name (FName)
							if (FNameProperty* NameP = CastField<FNameProperty>(PinStruct->FindPropertyByName(FName(TEXT("Name")))))
							{
								PinObj->SetStringField(TEXT("name"), NameP->GetPropertyValue_InContainer(PinPtr).ToString());
							}
							// TypeName (FName)
							if (FNameProperty* TypeP = CastField<FNameProperty>(PinStruct->FindPropertyByName(FName(TEXT("TypeName")))))
							{
								PinObj->SetStringField(TEXT("type_name"), TypeP->GetPropertyValue_InContainer(PinPtr).ToString());
							}
					...
					ExtractClassPins(TEXT("Inputs"),  InputsArray);
					ExtractClassPins(TEXT("Outputs"), OutputsArray);
				}
			}
		}
	...
	// Uses core Engine SoundCue/SoundNode APIs directly (no optional module).
	// -----------------------------------------------------------------------
	...
	// Uses core FAudioDevice API directly (no optional module).
	// -----------------------------------------------------------------------
